...
<div class="mb-4">
    <h2 class="fw-bold text-white">
        <i class="bi bi-plus-circle me-2"></i> Tambah Pembelajaran Baru
    </h2>
    <p class="text-white opacity-75">Buat materi pembelajaran untuk siswa</p>
</div>
                ...
                <form action="{{ route('lessons.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="judul" class="form-control @error('judul') is-invalid @enderror" 
                               placeholder="Contoh: Sejarah Agama Hindu di Indonesia" value="{{ old('judul') }}" required>
                        @error('judul')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="deskripsi" rows="3" class="form-control @error('deskripsi') is-invalid @enderror" 
                                  placeholder="Tulis deskripsi singkat tentang pembelajaran ini..." required>{{ old('deskripsi') }}</textarea>
                        @error('deskripsi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="konten" rows="10" class="form-control @error('konten') is-invalid @enderror" 
                                  placeholder="Tulis materi pembelajaran secara lengkap..." required>{{ old('konten') }}</textarea>
                        @error('konten')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="text-muted">
                            <i class="bi bi-info-circle me-1"></i> Tulis materi pembelajaran dengan jelas dan mudah dipahami siswa
                        </small>
                    </div>
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-save me-2"></i> Simpan Pembelajaran
                        </button>
